...
console.log(
	"🔧 Fixing Database Structure - Consolidating Individual Records...",
);
...
async function main() {
	try {
		...
		try {
			...
			if (Array.isArray(countResult) && countResult.length > 0) {
				...
				if (Array.isArray(firstResult)) {
					console.log(
						`📊 Found ${firstResult.length} individual count records`,
					);
					console.log(
						"📊 This confirms messages are stored as individual records",
					);
				}
			}
		} catch (error) {
		...
		console.log(
			`📊 Found ${currentMessages.length} messages via getMessages()`,
		);
      ...
      `);
        ...
        `,
				...
				console.log(
					`⚠️  Failed to insert message ${message.id}:`,
					error.message,
				);
			}
		}
		...
		try {
			...
			console.log(
				"📊 New count result:",
				JSON.stringify(newCountResult, null, 2),
			);
			...
			if (Array.isArray(newCountResult) && newCountResult.length === 1) {
				...
				if (typeof count === "object" && "count" in count) {
					console.log(
						`✅ SUCCESS: Proper count result - ${count.count} messages`,
					);
				} else {
				...
				console.log(
					"⚠️  Count result still has multiple items:",
					newCountResult.length,
				);
			}
		} catch (error) {
