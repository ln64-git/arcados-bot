...
async function main() {
  try {
    ...
    try {
      const tables = await db.db.query('SELECT * FROM INFORMATION_SCHEMA.TABLES');
    ...
    try {
      ...
      const result = await db.db.query('SELECT * FROM messages:1254694808228986912:* LIMIT 5');
      console.log('📨 Individual message records:', JSON.stringify(result, null, 2));
    } catch (error) {
