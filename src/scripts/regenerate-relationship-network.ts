...
async function regenerateRelationshipNetwork() {
	console.log(
		"🔹 Regenerating relationship network from message interactions...",
	);
	...
	try {
		...
		for (const member of membersResult.data) {
			...
			const relationships: RelationshipEntry[] = interactionsResult.data.map(
				(row) => ({
			...
			if (relationships.length > 0) {
				...
				const updateResult = await db.updateMemberRelationshipNetwork(
					memberId,
					relationships,
				);
				...
				if (updateResult.success) {
					console.log(
						`✅ Updated ${relationships.length} relationships for ${member.username}`,
					);
				} else {
					console.error(
						`🔸 Failed to update relationships for ${member.username}: ${updateResult.error}`,
					);
				}
			} else {
				console.log(
					`🔹 No significant relationships found for ${member.username}`,
				);
			}
			...
			if (processedMembers % 50 === 0) {
				console.log(
					`🔹 Processed ${processedMembers}/${membersResult.data.length} members...`,
				);
			}
		}

		console.log(
			`✅ Relationship network regeneration completed for ${processedMembers} members`,
		);
