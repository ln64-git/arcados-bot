...
async function main() {
	try {
		...
		console.log(
			`   Percentage of all messages: ${((targetUserMessages.length / messages.length) * 100).toFixed(1)}%`,
		);
		...
		const sortedChannels = Object.entries(channelStats).sort(
			([, a], [, b]) => b.total - a.total,
		);
		...
		for (const [channelId, stats] of sortedChannels) {
			...
			console.log(
				`     Target user messages: ${stats.targetUser} (${percentage}%)`,
			);
		}
		...
		const avgLength =
			targetUserContent.reduce((sum, content) => sum + content.length, 0) /
			targetUserContent.length;
		console.log(
			`   Average message length: ${avgLength.toFixed(1)} characters`,
		);

		const shortMessages = targetUserContent.filter(
			(content) => content.length < 10,
		);
		const longMessages = targetUserContent.filter(
		...
		for (const [userId, stats] of sortedUsers) {
			...
			console.log(
				`${marker} User ${userId}: ${stats.count} messages, ${stats.channels.size} channels`,
			);
		}
		...
		console.log(
			`   Date range: ${oldestMessage.toDateString()} to ${newestMessage.toDateString()}`,
		);
		console.log(`   Time span: ${dateRange.toFixed(1)} days`);

		if (dateRange < 7) {
			console.log(
				`   ⚠️  WARNING: Very short time span - sync might be incomplete`,
			);
		}
