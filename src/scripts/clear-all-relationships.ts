...
async function clearAllRelationshipData(guildId?: string, confirm = false): Promise<ClearStats> {
		...
		if (!confirm) {
			...
			for (const member of members.slice(0, 5)) { // Show first 5 as preview
				...
				relationships.slice(0, 3).forEach((rel: any, index: number) => {
					console.log(`   ${index + 1}. User ${rel.user_id}: ${rel.affinity_percentage}% affinity, ${rel.interaction_count} interactions`);
				});
			...
			if (allMembersResult.success && allMembersResult.data) {
				const totalAllRelationships = allMembersResult.data.reduce((sum: number, member: any) => {
			...
			console.log("⚠️  This includes affinity_percentage, interaction_count, last_interaction, and all metadata");
			console.log("⚠️  User profiles (summary, keywords, emojis) will be preserved");
			console.log("\nTo proceed, run with --confirm flag");
			console.log("Example: npx tsx clear-all-relationships.ts --confirm");
			if (guildId) {
				console.log(`Example: npx tsx clear-all-relationships.ts ${guildId} --confirm`);
			}
			...
			try {
				...
				if (updateResult.success) {
					...
					if (relationshipCount > 0) {
						console.log(`✅ ${member.display_name}: cleared ${relationshipCount} relationships`);
					}
				} else {
...
async function regenerateRelationshipNetworks(guildId?: string): Promise<void> {
	...
	try {
		...
		for (const member of members) {
			...
			`, [JSON.stringify(relationships), userId, guildId || '1254694808228986912']);

			if (updateResult.success) {
				console.log(`✅ ${member.username}: generated ${relationships.length} relationships`);
			} else {
				console.log(`🔸 Failed to update ${member.username}: ${updateResult.error}`);
			}
			...
			if (processedMembers % 10 === 0) {
				console.log(`🔹 Progress: ${processedMembers}/${members.length} members processed`);
			}
		}
...
async function main() {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg === '--confirm') {
			...
			console.log("Usage:");
			console.log("  npx tsx clear-all-relationships.ts [guild-id] [--confirm] [--regenerate]");
			console.log("");
			...
			console.log("  --regenerate  Regenerate relationship networks after clearing");
			console.log("");
			console.log("Examples:");
			console.log("  npx tsx clear-all-relationships.ts                    # Preview all guilds");
			console.log("  npx tsx clear-all-relationships.ts --confirm          # Clear all guilds");
			console.log("  npx tsx clear-all-relationships.ts --confirm --regenerate # Clear and regenerate");
			console.log("  npx tsx clear-all-relationships.ts 123456789 --confirm # Clear specific guild");
			console.log("");
			console.log("This script COMPLETELY CLEARS all relationship data including:");
