...
async function checkRealDiscordMessages() {
	...
	try {
		...
		console.log(
			`🔹 Real Discord messages (excluding test data): ${realMessages.length}`,
		);

		const userId = "99195129516007424";
		const userRealMessages = realMessages.filter(
			(msg) => msg.author_id === userId,
		);
		console.log(
			`🔹 Real messages from user ${userId}: ${userRealMessages.length}`,
		);

		if (userRealMessages.length > 0) {
			userRealMessages.sort(
				(a, b) =>
			...
			console.log(
				"🔹 This suggests Discord sync hasn't captured real messages yet",
			);
		}
		...
		realMessages.forEach((msg, i) => {
			console.log(
				`${i + 1}. [${new Date(msg.timestamp).toLocaleString()}] User: ${msg.author_id}`,
			);
			console.log(`   Content: "${msg.content}"`);
			console.log(`   Channel: ${msg.channel_id}`);
			console.log();
