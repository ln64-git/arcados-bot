...
app.get('/api/members/:userId/relationships', async (req, res) => {
    try {
        ...
        res.json({
            ...
            total_percentage: network.reduce((sum, rel) => sum + rel.affinity_percentage, 0),
    ...
    try {
        ...
        res.json({
            ...
            stats: {
                ...
                members_without_networks: parseInt(totalMembers.rows[0].count) - parseInt(membersWithNetworks.rows[0].count),
                coverage_percentage: ((parseInt(membersWithNetworks.rows[0].count) / parseInt(totalMembers.rows[0].count)) * 100).toFixed(2)
            }
        });
    } catch (error) {
        console.error('Error:', error);
...
app.listen(PORT, () => {
    console.log(`🔹 Relationship Network API server running on http://localhost:${PORT}`);
