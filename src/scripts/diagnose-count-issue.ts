...
async function main() {
	try {
		...
		try {
			...
			if (Array.isArray(countResult) && countResult.length > 0) {
				...
				if (Array.isArray(firstResult)) {
					console.log(
						`📊 First result is array with ${firstResult.length} items`,
					);
		...
		try {
			...
			const directMessages = await db.db.query(
			...
			if (Array.isArray(directMessages) && directMessages.length > 0) {
				...
				if (Array.isArray(firstMessage)) {
					...
					console.log(
						`📊 Found ${firstMessage.length} individual message records`,
					);
				} else {
		...
		try {
			...
			const individualPattern = await db.db.query(
				"SELECT * FROM messages:* LIMIT 5",
			);
			console.log(
				"📊 Individual pattern query:",
				JSON.stringify(individualPattern, null, 2),
			);
		} catch (error) {
