...
async function main() {
  ...
  try {
    ...
    const completedChannels = [
      'selfies-irl', 'lush', 'ai', 'gaming', 'news', 'butterz', 
      'temagami-x-frankie', 'ant-hill', 'alex', 'youtube', 'polls', 'америка'
    ];
    ...
    console.log(`   ✅ Channels processed: ${processedChannels}/${textChannels.size - completedChannels.length}`);
    console.log(`   🔸 Failed channels: ${failedChannels.length}`);
    console.log(`   📈 Total messages in database: ${totalMessages + 5352}+ (including previous sync)`);
