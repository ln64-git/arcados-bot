...
async function clearRelationshipMetadata(
	guildId?: string,
	confirm = false,
): Promise<ClearStats> {
	console.log(
		"🔹 Clearing relationship metadata (summary, keywords, emojis) from all users...",
	);
	...
	try {
		...
		if (!confirm) {
			...
			for (const member of members.slice(0, 5)) {
				...
				relationships.slice(0, 3).forEach((rel: any, index: number) => {
					const hasMetadata =
						rel.summary ||
						(rel.keywords && rel.keywords.length > 0) ||
						(rel.emojis && rel.emojis.length > 0);
					console.log(
						`   ${index + 1}. User ${rel.user_id}: ${hasMetadata ? "Has metadata" : "No metadata"}`,
					);
				});
			...
			console.log(
				"⚠️  The relationship data (affinity_percentage, interaction_count, last_interaction) will be preserved",
			);
			console.log("\nTo proceed, run with --confirm flag");
			console.log("Example: npx tsx clear-relationship-metadata.ts --confirm");
			if (guildId) {
				console.log(
					`Example: npx tsx clear-relationship-metadata.ts ${guildId} --confirm`,
				);
			}
		...
		for (const member of members) {
			try {
				...
				`,
					[
						JSON.stringify(cleanedRelationships),
						member.user_id,
						member.guild_id,
					],
				);
				...
				if (updateResult.success) {
					...
					if (relationshipsCleared > 0) {
						console.log(
							`✅ ${member.display_name}: cleared ${relationshipsCleared}/${relationships.length} relationships`,
						);
					}
				} else {
					console.log(
...
async function main() {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg === "--confirm") {
			...
			console.log("Usage:");
			console.log(
				"  npx tsx clear-relationship-metadata.ts [guild-id] [--confirm]",
			);
			console.log("");
			...
			console.log(
				"  npx tsx clear-relationship-metadata.ts                    # Preview all guilds",
			);
			console.log(
				"  npx tsx clear-relationship-metadata.ts --confirm          # Clear all guilds",
			);
			console.log(
				"  npx tsx clear-relationship-metadata.ts 123456789 --confirm # Clear specific guild",
			);
			console.log("");
			console.log(
				"This script clears summary, keywords, and emojis from relationship entries",
			);
			console.log(
				"while preserving affinity_percentage, interaction_count, last_interaction, and notes.",
			);
			process.exit(0);
