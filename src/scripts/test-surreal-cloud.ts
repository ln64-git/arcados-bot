...
async function testSurrealCloudConnection() {
	...
	console.log(`   SURREAL_NAMESPACE: ${process.env.SURREAL_NAMESPACE || 'NOT SET'}`);
	console.log(`   SURREAL_DATABASE: ${process.env.SURREAL_DATABASE || 'NOT SET'}`);
	console.log(`   SURREAL_USERNAME: ${process.env.SURREAL_USERNAME || 'NOT SET'}`);
	console.log(`   SURREAL_TOKEN: ${process.env.SURREAL_TOKEN ? 'SET' : 'NOT SET'}`);
	...
	try {
		...
		console.log("   1. Make sure you have a .env file with your SurrealDB Cloud credentials");
		console.log("   2. Check that SURREAL_URL is correct (should start with wss://)");
