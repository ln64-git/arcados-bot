...
async function main() {
  try {
    ...
    console.log(`🔹 Target user ${TARGET_USER_ID} has ${targetUserMessages.length} messages`);
    ...
    const relationships = await relationshipManager.getUserRelationships(TARGET_USER_ID);
    ...
    if (relationships.length > 0) {
      ...
      const details = await relationshipManager.getRelationshipDetails(TARGET_USER_ID, topUser);
