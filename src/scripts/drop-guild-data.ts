...
async function dropGuildData() {
	...
	try {
		...
		const membersResult = await db.query("DELETE FROM members WHERE guild_id = $1", [guildId]);
		console.log(`✅ Deleted ${membersResult.data?.length || 0} members`);

		console.log("🔹 Dropping roles...");
		const rolesResult = await db.query("DELETE FROM roles WHERE guild_id = $1", [guildId]);
		console.log(`✅ Deleted ${rolesResult.data?.length || 0} roles`);

		console.log("🔹 Dropping channels...");
		const channelsResult = await db.query("DELETE FROM channels WHERE guild_id = $1", [guildId]);
		console.log(`✅ Deleted ${channelsResult.data?.length || 0} channels`);

		console.log("🔹 Dropping guild...");
		const guildResult = await db.query("DELETE FROM guilds WHERE id = $1", [guildId]);
