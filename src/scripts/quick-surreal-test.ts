...
async function quickTest() {
	...
	try {
		...
		const userMessages = messages.filter((msg: any) => msg.author_id === userId);
		console.log(`✅ Found ${userMessages.length} messages from user ${userId}`);
		
		if (userMessages.length > 0) {
			userMessages.sort((a: any, b: any) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
