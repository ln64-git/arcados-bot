...
export const undeafenCommand: Command = {
	data: new SlashCommandBuilder()
		.setName("undeafen")
		.setDescription("Remove deafen from a user in your voice channel")
	...
	async execute(interaction) {
		...
		try {
			...
			// Remove deafen
			const undeafenResult = await voiceChannelManager.removeDeafen(
				voiceChannel.id,
				targetUser.id,
			);
			if (!undeafenResult.success) {
				await interaction.reply({
					content: `🔸 Failed to undeafen user: ${undeafenResult.error}`,
					ephemeral: true,
				});
				return;
			}
			...
			await interaction.reply({
				content: `🔹 Undeafened **${targetUser.displayName}** in this channel.`,
				ephemeral: false,
			});
		} catch (error) {
			console.error("🔸 Error in undeafen command:", error);
			await interaction.reply({
				content: "🔸 An error occurred while trying to undeafen the user.",
				ephemeral: true,
			});
		}
	},
