...
export const unmuteCommand: Command = {
	data: new SlashCommandBuilder()
		.setName("unmute")
		.setDescription("Remove mute from a user in your voice channel")
	...
	async execute(interaction) {
		...
		try {
			...
			// Remove mute
			const unmuteResult = await voiceChannelManager.removeMute(
				voiceChannel.id,
				targetUser.id,
			);
			if (!unmuteResult.success) {
				await interaction.reply({
					content: `🔸 Failed to unmute user: ${unmuteResult.error}`,
					ephemeral: true,
				});
				return;
			}
