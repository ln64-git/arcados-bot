...
import { RelationshipNetworkManager } from "../NetworkManager";
  ...
  try {
    ...
    for (let i = 0; i < members.length; i++) {
      ...
      try {
        ...
        stats.failed_updates++;
        const errorMsg = `Exception updating ${member.user_id}: ${
    ...
    console.log(
      `   - Average per member: ${Math.round(
        stats.total_duration_ms / stats.processed_members
      )}ms`
    );
