...
async function main() {
	try {
		...
		const url =
			process.env.SURREAL_URL || "wss://your-project.surrealdb.com/rpc";
		...
		try {
			...
			console.log(
				...
				JSON.stringify(currentCount, null, 2),
			);
			...
			if (Array.isArray(currentCount) && currentCount.length > 0) {
				...
				if (Array.isArray(firstResult)) {
					console.log(
						`📊 Found ${firstResult.length} individual count records (problematic)`,
					);
				} else {
		...
		try {
			...
			console.log(
				"⚠️  Could not remove messages table (may not exist):",
				error.message,
			);
		}
      ...
      `,
		...
		console.log(
			"🚀 You can now run the Discord sync to populate the properly structured table.",
		);
	} catch (error) {
