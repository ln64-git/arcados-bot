...
export class ReconciliationSync {
	...
	async runOnce(): Promise<void> {
		...
		try {
			...
			console.error("🔸 ReconciliationSync: Error during reconciliation:", error);
		}
	}
	...
	private async reconcileGuild(guild: Guild): Promise<void> {
		...
		try {
			...
			if (!guildResult.success) {
				console.error(`🔸 ReconciliationSync: Failed to upsert guild ${guildId}`);
				return;
			}
		} finally {
			releaseLock();
	...
	private async reconcileChannels(guild: Guild): Promise<void> {
		...
		if (this.verbose) {
			console.log(`   📝 ReconciliationSync: Syncing ${channels.length} channels...`);
		}
	...
	private async reconcileMembers(guild: Guild): Promise<void> {
		...
		if (this.verbose) {
			console.log(`   👥 ReconciliationSync: Syncing ${membersArray.length} members...`);
		}
	...
	private async reconcileMessages(guild: Guild): Promise<void> {
		...
		if (this.verbose) {
			console.log(`   💬 ReconciliationSync: Checking ${channels.length} channels for gaps...`);
		}
	...
	private async backfillChannel(
		...
		if (this.verbose) {
			console.log(`   📥 ReconciliationSync: Backfilling channel ${channelName}...`);
		}
		...
		if (synced > 0) {
			...
			if (this.verbose) {
				console.log(`   ✅ ReconciliationSync: Backfilled ${synced} messages in ${channelName}`);
			}
		}
	}
	...
	private async consolidateOverlappingSegments(): Promise<void> {
		try {
			...
			if (consolidated > 0 && this.verbose) {
				console.log(`🔹 ReconciliationSync: Consolidated ${consolidated} overlapping segments`);
			}
		} catch (error) {
