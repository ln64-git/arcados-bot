...
export class VoiceConnectionManager {
  ...
  private audioReceiveCallbacks: Map<
    Snowflake,
    (userId: string, audioData: Buffer, speakingStartTime?: number) => void
  > = new Map();
  ...
  public startReceiving(
    guildId: Snowflake,
    onAudioReceived: (userId: string, audioData: Buffer, speakingStartTime?: number) => void
  ): void {
    const session = this.sessions.get(guildId);
    ...
    connection.receiver.speaking.on("start", (userId) => {
      const speakingStartTime = Date.now(); // Capture when Discord detects user speaking
      this.logger.debug(
      ...
      decoder.on("end", () => {
        if (audioChunks.length > 0) {
          ...
          if (currentCallback) {
            currentCallback(userId, combinedAudio, speakingStartTime);
          } else {
            this.logger.warn(
